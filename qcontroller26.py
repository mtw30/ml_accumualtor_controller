class qcontroller26:
    'Made sure it was calcualting the reward for all terminal states'
    ...
    def __init__(self):
        ...
        self.aimTemperature = float(30 + 273)
        self.minAccel = 0.0
        self.maxAccel = 183.0
        self.minGenHeat = 0.0
        ...
        self.minBattCap = 0.0
        self.maxSTemp = 1500.0
        self.minSTemp = -1500.0
        ...
        self.dataCount = 14       # Number of data values to learn from per row
        self.dataDepth = 2       # The total number of rows per argent, oldest is popped off
        self.outputDepth = 6     

        # Load our created model!
        jsonFile = open('ml_attempt_26/qrienforcmentmodel.json', 'r')
        loadedModelJSON = jsonFile.read()
        jsonFile.close()
        self.model = model_from_json(loadedModelJSON)
        # Load the weights into thr new model
        self.model.load_weights("ml_attempt_26/qrienforcmentmodel.h5")
    ...
    def runController(self, lineData, dt, acceleration, temperature, generatedHeat, fanSpeed, remainCapacity):
        ...
        self.sTemp = self.sTemp + (temperature - self.aimTemperature) * dt
        normsTemp = 2 * (self.sTemp - self.minSTemp) / (self.maxSTemp - self.minSTemp) - 1
        ...
        dSped = (dataRow[3] - self.state[1][3]) / normDT
        dTemp = (dataRow[4] - self.state[1][4]) / normDT
        dHeat = (dataRow[6] - self.state[1][6]) / normDT
        dFans = (dataRow[7] - self.state[1][7]) / normDT
