...
def processFile(filePath):
    ...
    epochs = 5                   # Run the learn 1000 times per simulation
    epsilon = 1                     # Using epsilon greedy, decreases over time
    gamma = 0.8                     # Used in the Q learning algorithm equation
    batchSize = 10                  # How many pieces of data to use per network train
    bufferSize = 20                 # Amount of values to fill the buffer with
    replay = []                     # Stores tuples of (s, a, r, s')
    h = 0                           # Counter for the index to insert the buffered state
    ...
    for i in range(epochs):
        ...
        while isRunning:
            ...
            # Begin experience replay storage
            if len(replay) < bufferSize:
                # The buffer is not yet full, so just add to it
                # WE dont process anything until the buffer is full
                replay.append((state, actionIndex, reward, nextState))
            else:
                # The buffer is full, loop over deleting old values
                if h < (bufferSize - 1):
                    h += 1
                else:
                    h = 0

                # Store this new value on top of an old value
                replay[h] = (state, actionIndex, reward, nextState)

                # Randomly sample from the expierience replay mem
                randBatch = random.sample(replay, batchSize)
                Xtrain = []
                Ytrain = []

                for selMemory in randBatch:
                    # Get the max Q'(s', a)
                    oldState, action, reward, newState = selMemory
                    oldQ = model.predict(np.array(oldState).reshape(1,(dataCount * dataDepth)), batch_size=1)
                    newQ = model.predict(np.array(newState).reshape(1,(dataCount * dataDepth)), batch_size=1)
                    maxQ = np.max(newQ)
                    ...
                    y[0][action] = update
                    Xtrain.append(np.array(oldState).reshape((dataCount * dataDepth), ))
                    Ytrain.append(y.reshape(outputDepth,))

                Xtrain = np.array(Xtrain)
                Ytrain = np.array(Ytrain)
                model.fit(Xtrain, Ytrain, batch_size=batchSize, nb_epoch=1, verbose=0)
            # End expirence replay storage
