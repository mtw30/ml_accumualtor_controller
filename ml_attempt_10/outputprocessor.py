...
trnlEx = re.compile(r'End training loss : (\d+\.?\d*)') 
rewdEx = re.compile(r'End reward : (-?\d+\.?\d*)') 
...
with open("output.txt") as f:
    for line in f:
        ...
        if trnlEx.search(processLine):
            printLine += str(trnlEx.search(processLine).groups(1)[0]) + ","

        if rewdEx.search(processLine):
            printLine += str(rewdEx.search(processLine).groups(1)[0]) + ","
