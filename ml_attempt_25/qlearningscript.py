...
dataCount = 14      # Number of data values to learn from per row
dataDepth = 2       # The total number of rows per argent, oldest is popped off
outputDepth = 6     
...
def initState(initialLines):
    ...
    for line in initialLines:
        ...
        dataRow.append(0.0)             # For the data derivatives
        dataRow.append(0.0)
        dataRow.append(0.0)
        dataRow.append(0.0)
        dataRow.append(0.0)             # Tempertautre error intergral
...
def calculateCooling(coolingThrottle, dt):
    ...
    # Because now 6 piece of data have been averaged over
    cellCount *= 1
...
# battery temp, current line, and the next data line, as well as the intergral of the battery temp error
# Returns the next state, as well as raw values for temperature as this is used to calculate the reward
def advanceState(state, fanSpeed, temp, thisBattCapacity , currLine, nextLine, sTemp):
    ...
    #Using q=m*c*Delta 
    dTemp = float(instantaneousPower) / (float(battMass) * float(specHeatCap) )

    # Change the current temperature by the temperature change calculated
    temp = temp + dTemp
    #print("Temperature : " + str(temp - 273))

    # Comute the temeprature intergral
    print('Temp difference : ' + str(temp - aimTemperature))
    sTemp = sTemp + (temp - aimTemperature) * dt
    normsTemp = 2 * (sTemp - minSTemp) / (maxSTemp - minSTemp) - 1
    ...
    # Work out some tasty derivatives
    dSped = (dataRow[3] - newState[0][3]) / normDT
    dTemp = (dataRow[4] - newState[0][4]) / normDT
    dHeat = (dataRow[6] - newState[0][6]) / normDT
    dFans = (dataRow[7] - newState[0][7]) / normDT

    dataRow.append(dSped)
    dataRow.append(dTemp)
    dataRow.append(dHeat)
    dataRow.append(dFans)

    # Append the temperature error intergral
    dataRow.append(normsTemp)

    newState.append(dataRow)

    return newState, temp, nextBattCap, sTemp
...
def calculateReward(battTemp, newBattTemp, thisBattCapacity, newBattCap, sTemp):

    # Max is actually 11, but want to encourage a nice +2 reward
    bottomReward = -6
    maxReward = 6.0
    ...
    # Changes the 0.5 to 1/3 and added the 1/6 because of the 6 point averaging of data
    drainDiscountFactor = perDiff * 100000.0 * (1.0/3.0) * (1.0 / 6.0) * 0.25 * 0.5
    # Need to put a lower bound on this as it sometimes spikes to -165
    # Only keeping at -6 as that is rougly the bottom it is usually set to (with the other -2)
    drainDiscountFactor = 6 if drainDiscountFactor > 6 else drainDiscountFactor
    #reward -= drainDiscountFactor
    ...
    multiplier = 14.0        # Multipy the standard distribution to give better ANN teachings
    ...
    # For this run have the reward based on the intergral of the battery temp error
    normsTemp = 2 * (sTemp - minSTemp) / (maxSTemp - minSTemp) - 1
    reward += 4 - 4 * abs(normsTemp)

    #print(reward)
    # Normalise the reward to this point
    # The other rewards can be bigger as they're termanal states
    reward = (2.0 * (reward - bottomReward) / (maxReward - bottomReward)) - 1
...
def processFile(filePath):
    ...
    epochs = 1                   # Run the learn 1000 times per simulation
    epsilon = 1                     # Using epsilon greedy, decreases over time
    gamma = 0.8                     # Used in the Q learning algorithm equation
    lastQVal = 0.0
    ...
    for i in range(epochs):
        ...
        dFan = 1.0
        dSpeed = 1.0
        # Inintialise the intergral temp value
        sTemp = 1.0
        ...
        while isRunning:
            ...
            nextState, nextBattTemp, nextBattCap, sTempp1 = \
                    advanceState(state, nextFanSpeed, battTemp, thisBattCapacity, currLine, readLines[0].split(','), sTemp)
            ...
            reward = calculateReward(battTemp, nextBattTemp, thisBattCapacity, nextBattCap, sTempp1) 
            # Keep a hold of the total reward to output it
            runTotalReward += reward
            ...
            if len(readLines) == 1:
                ...
                isRunning = False
                update = reward + runTotalReward
            elif battTemp > maxAllowTemp or battTemp < minAllowTemp:
            ...
            fitData = model.fit(np.array(state).reshape(1, (dataCount * dataDepth)), y, batch_size=1, nb_epoch=1, verbose=0)

            # Incremeemnt the loss from training this data, to keep track of it
            runTotalLoss += fitData.history['loss'][0]
        ...
        # These new bits are added here so the capacity is still at the end for the .csv processsing script
        print("End training loss : " + str(runTotalLoss))
        print("End reward : " + str(runTotalReward))
        print("Final qVal : " + str(qVal))
