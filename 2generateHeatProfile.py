...
#***** THe number 2 is simply because the dt multiplier on the q=mc^t equation has been removed
...
controller = False      # Insantiated object of a controller specified by the command line argument
...
def calculateCooling(coolingThrottle, dt):
    ...
    # As we are now working over 6 data values
    cellCount *= 2.0
...
def processFile(filePath):
    ...
    if verbOutput:
        ...
        fileName = str(sys.argv[1]) + "_" + fileName
    ...
    temp = Tamb
    # Cooling throttle
    coolingThrottle = 0.0       # The actual cooling value 0 - 1 being send to the fans
    setCool = 0.0               # What the coolingThrottle wants to be, but emulates fan spinning up/down
    ...
    for line in processLine[1:]:
        ...
        #############################################################################
        #Run the instantiated controller, if it is actiavated
        if controller != False:
            setCool = controller.runController(lineData, dt, acceleration, temp,
                    generatedHeat, coolingThrottle, thisBattCapacity)
        #############################################################################
        ...
        coolingThrottle = changeCoolingThrottle(coolingThrottle , setCool, dt)
        ...
        if (temp < minTemp or temp > maxTemp) and not verbOutput:
        ...
        if False:
            ...
            print("Throttle value : " + str(coolingThrottle))
...
if (len(sys.argv) > 1):
    if sys.argv[1] == "-verb":
        ...
        controllerClass = getattr(module, sys.argv[1])
