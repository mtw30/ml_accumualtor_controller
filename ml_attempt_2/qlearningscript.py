...
def calculateReward(battTemp, newBattTemp, thisBattCapacity, newBattCap):
    ...
    multiplier = 10.0        # Multipy the standard distribution to give better ANN teachings

    reward += (1.0 / (sigma * (2.0 ** math.pi) ** 0.5)) * \
            math.exp(-1.0 * ((newBattTemp - mean) ** 2.0) / (2 * newBattTemp ** 2.0)) * multiplier
